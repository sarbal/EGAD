...
#' @export
#'
plot_value_compare <- function(aucA, aucB, xlab = "AUROC", ylab = "AUROC", xlim = c(0, 1), ylim = c(0, 1)) {
    
    a <- mean(aucA, na.rm = T)
    b <- mean(aucB, na.rm = T)
    plot(aucA, aucB, xlim = xlim, ylim = ylim, xlab = xlab, ylab = ylab, bty = "n", cex.lab = 1.5, cex.axis = 1.2, 
        pch = 19)
